function [Vseq, Dseq, Wseq] = fn_eigenshuffle_rl(Asequence)
% My edits are marked as 'fix'.
...
for i = 2:n
  ...
  Vseq(:,S,i) = -Vseq(:,S,i);
  % fix
  % S = squeeze(real(sum(Wseq(:,:,i-1).*Wseq(:,:,i),1))) < 0;
  % Wseq(:,S,i) = -Wseq(:,S,i);
  Wseq(:,:,i) = inv(Vseq(:,:,i));
end
